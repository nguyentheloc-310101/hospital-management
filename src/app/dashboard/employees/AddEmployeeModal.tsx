...
const AddEmployeeModal = ({visible, onCancel, onOk, confirmLoading}) => {
    ...
    return (
        ...
        >
            ...
            <DatePicker size={'large'} value={birthdate} onChange={(date) => {
                setBirthdate(date)
            }}></DatePicker>
            <Title level={5}>Gender</Title>
            <Dropdown.Button
                overlay={<Menu items={genders} onClick={handleGenderClick}/>}
                trigger={['click']}
                icon={<DownOutlined/>}
            >
            ...
            }} placeholder={'Enter phone number'} value={phoneNumber} onChange={(phoneNumber) => {
                setPhoneNumber(phoneNumber.target.value)
            }}></Input>

            <Title level={5}>Degree</Title>
            <Input value={degree} onChange={(e) => setDegree(e.target.value)} placeholder={'Enter degree name'}/>
            <Title level={5}>Degree Year</Title>
            <Input onKeyPress={(event) => {
                if (!/[0-9]/.test(event.key)) {
                    event.preventDefault();
                }
            }} value={degreeYear} onChange={(e) => setDegreeYear(e.target.value)} placeholder={'Enter degree year'}/>
            <Title level={5}>Start Date</Title>
            <DatePicker size={'large'} value={startDate} onChange={(date) => {
                setStartDate(date)
            }}></DatePicker>
            <Title level={5}>Role</Title>
            <Dropdown.Button
                overlay={<Menu items={roles} onClick={handleRolesClick}/>}
                trigger={['click']}
                icon={<DownOutlined/>}
            >
            ...
            <Dropdown.Button
                overlay={<Menu items={departments} onClick={handleDeparmentClick}/>}
                trigger={['click']}
                icon={<DownOutlined/>}
            >
